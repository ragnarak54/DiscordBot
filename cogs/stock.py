...
class Stock(commands.Cog):
    ...
    @app_commands.command(name='worlds')
    async def worlds(self, interaction: discord.Interaction):
        """Shows any worlds with active merchant"""
        w = self.bot.get_cog('WorldTracker').worlds
        if w:
            await interaction.response.send_message(f"Current world(s): {', '.join([f'**{_}**' for _ in w])}")
        else:
            await interaction.response.send_message("No current worlds")
